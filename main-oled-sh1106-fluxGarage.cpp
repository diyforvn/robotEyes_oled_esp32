...
void setup()   {
  ...
  // roboEyes.setSweat(ON); //> when turned on, animated sweat drops appear in the upper screen area
} // end of setup


void loop() {
 roboEyes.update(); // update eyes drawings
  if (millis() - moodTimer > moodChangeInterval) {
    currentMood = (currentMood + 1) % 5;  //8 mode
