...
#define ANGRY 2
#define HAPPY 3
#define HAPPYHEART 4 // Heart eyes mood
#define LIGHTNINGEYES 5 // Mắt hình sấm sét
#define DOLLAREYES 6 // Mắt hình đồng đô la
#define CLOCKEYES 7 // Mắt hình đồng hồ
...
bool angry = 0;
bool happy = 0;
bool happyHeart = 0; // New variable for heart eyes
bool lightningEyes = 0; // Thêm biến cho mắt sấm sét
bool dollarEyes = 0;    // Thêm biến cho mắt đồng đô la
bool clockEyes = 0;    // Thêm biến cho mắt đồng hồ
...
{
  // Đặt tất cả các trạng thái mắt về 0
  tired = 0;
  angry = 0;
  happy = 0;
  happyHeart = 0;
  ...
  case CLOCKEYES:
    ...
    break;
  default:
    // Mọi thứ đã được đặt lại ở trên
    break;
  }
}
...
void drawEyes(){
  ...
  // Draw basic eye rectangles or heart shape based on mood
  // Chuẩn bị các giá trị trước khi vẽ
  // Cần đặt lại kích thước mắt về mặc định nếu đang ở chế độ đặc biệt
  if (!happyHeart && !lightningEyes && !dollarEyes && !clockEyes) {
  ...
  int heartLX = eyeLx;
  int heartLY = eyeLy;
  int heartWidth = eyeLwidthCurrent;
  int heartHeight = eyeLheightCurrent;
  
  // Two circles at the top
  display->fillCircle(heartLX + heartWidth/4, heartLY + heartHeight/4, heartWidth/4, MAINCOLOR);
  display->fillCircle(heartLX + 3*heartWidth/4, heartLY + heartHeight/4, heartWidth/4, MAINCOLOR);
  
  // A triangle at the bottom
  display->fillTriangle(heartLX, heartLY + heartHeight/4, heartLX + heartWidth, heartLY + heartHeight/4, heartLX + heartWidth/2, heartLY + heartHeight, MAINCOLOR);

  // Draw right heart eye
  if (!cyclops){
    int heartRX = eyeRx;
    int heartRY = eyeRy;
    int heartRWidth = eyeRwidthCurrent;
    int heartRHeight = eyeRheightCurrent;
    
    // Two circles at the top
    display->fillCircle(heartRX + heartRWidth/4, heartRY + heartRHeight/4, heartRWidth/4, MAINCOLOR);
    display->fillCircle(heartRX + 3*heartRWidth/4, heartRY + heartRHeight/4, heartRWidth/4, MAINCOLOR);
    
    // A triangle at the bottom
    display->fillTriangle(heartRX, heartRY + heartRHeight/4, heartRX + heartRWidth, heartRY + heartRHeight/4, heartRX + heartRWidth/2, heartRY + heartRHeight, MAINCOLOR);
  }
  } else if (lightningEyes) {
    // Logic vẽ mắt sấm sét
    // Mắt trái:
    display->drawLine(eyeLx, eyeLy + eyeLheightCurrent/2, eyeLx + eyeLwidthCurrent/3, eyeLy + eyeLheightCurrent/2, MAINCOLOR);
    display->drawLine(eyeLx + eyeLwidthCurrent/3, eyeLy + eyeLheightCurrent/2, eyeLx + eyeLwidthCurrent/3, eyeLy, MAINCOLOR);
    display->drawLine(eyeLx + eyeLwidthCurrent/3, eyeLy, eyeLx + 2*eyeLwidthCurrent/3, eyeLy + eyeLheightCurrent, MAINCOLOR);
    display->drawLine(eyeLx + 2*eyeLwidthCurrent/3, eyeLy + eyeLheightCurrent, eyeLx + 2*eyeLwidthCurrent/3, eyeLy + eyeLheightCurrent/2, MAINCOLOR);
    display->drawLine(eyeLx + 2*eyeLwidthCurrent/3, eyeLy + eyeLheightCurrent/2, eyeLx + eyeLwidthCurrent, eyeLy + eyeLheightCurrent/2, MAINCOLOR);

    // Mắt phải (nếu không phải cyclops):
    if (!cyclops){
      display->drawLine(eyeRx, eyeRy + eyeRheightCurrent/2, eyeRx + eyeRwidthCurrent/3, eyeRy + eyeRheightCurrent/2, MAINCOLOR);
      display->drawLine(eyeRx + eyeRwidthCurrent/3, eyeRy + eyeRheightCurrent/2, eyeRx + eyeRwidthCurrent/3, eyeRy, MAINCOLOR);
      display->drawLine(eyeRx + eyeRwidthCurrent/3, eyeRy, eyeRx + 2*eyeRwidthCurrent/3, eyeRy + eyeRheightCurrent, MAINCOLOR);
      display->drawLine(eyeRx + 2*eyeRwidthCurrent/3, eyeRy + eyeRheightCurrent, eyeRx + 2*eyeRwidthCurrent/3, eyeRy + eyeRheightCurrent/2, MAINCOLOR);
      display->drawLine(eyeRx + 2*eyeRwidthCurrent/3, eyeRy + eyeRheightCurrent/2, eyeRx + eyeRwidthCurrent, eyeRy + eyeRheightCurrent/2, MAINCOLOR);
    }
  } else if (dollarEyes) {
    // Logic vẽ mắt đồng đô la
      // Mắt trái:
      display->fillRoundRect(eyeLx, eyeLy, eyeLwidthCurrent, eyeLheightCurrent, eyeLborderRadiusCurrent, MAINCOLOR);
      display->drawCircle(eyeLx + eyeLwidthCurrent/2, eyeLy + eyeLheightCurrent/2, eyeLwidthCurrent/4, BGCOLOR);
      
      // Vẽ hai đường dọc cho biểu tượng đô la
      int dollarX = eyeLx + eyeLwidthCurrent/2;
      int dollarY = eyeLy + eyeLheightCurrent/4;
      int dollarH = eyeLheightCurrent/2;
      
      // Đường dọc thứ nhất
      display->drawFastVLine(dollarX - 1, dollarY, dollarH, BGCOLOR);
      // Đường dọc thứ hai
      display->drawFastVLine(dollarX + 1, dollarY, dollarH, BGCOLOR);
      ...
      if (!cyclops){
        ...
        display->drawCircle(eyeRx + eyeRwidthCurrent/2, eyeRy + eyeRheightCurrent/2, eyeRwidthCurrent/4, BGCOLOR);
        
        // Vẽ hai đường dọc cho biểu tượng đô la
        int dollarRX = eyeRx + eyeRwidthCurrent/2;
        int dollarRY = eyeRy + eyeRheightCurrent/4;
        int dollarRH = eyeRheightCurrent/2;

        // Đường dọc thứ nhất
        display->drawFastVLine(dollarRX - 1, dollarRY, dollarRH, BGCOLOR);
        // Đường dọc thứ hai
        display->drawFastVLine(dollarRX + 1, dollarRY, dollarRH, BGCOLOR);
      }    
  } else if (clockEyes) {
    // Logic vẽ mắt đồng hồ
    // Mắt trái:
    display->fillCircle(eyeLx + eyeLwidthCurrent/2, eyeLy + eyeLheightCurrent/2, eyeLwidthCurrent/2, MAINCOLOR);
    display->drawCircle(eyeLx + eyeLwidthCurrent/2, eyeLy + eyeLheightCurrent/2, eyeLwidthCurrent/3, BGCOLOR);
    display->drawFastHLine(eyeLx + eyeLwidthCurrent/2, eyeLy + eyeLheightCurrent/2, eyeLwidthCurrent/3, BGCOLOR);
    display->drawLine(eyeLx + eyeLwidthCurrent/2, eyeLy + eyeLheightCurrent/2, eyeLx + eyeLwidthCurrent/2 + eyeLwidthCurrent/4, eyeLy + eyeLheightCurrent/2 - eyeLwidthCurrent/8, BGCOLOR);

    // Mắt phải (nếu không phải cyclops):
    if (!cyclops){
      display->fillCircle(eyeRx + eyeRwidthCurrent/2, eyeRy + eyeRheightCurrent/2, eyeRwidthCurrent/2, MAINCOLOR);
      display->drawCircle(eyeRx + eyeRwidthCurrent/2, eyeRy + eyeRheightCurrent/2, eyeRwidthCurrent/3, BGCOLOR);
      display->drawFastHLine(eyeRx + eyeRwidthCurrent/2, eyeRy + eyeRheightCurrent/2, eyeRwidthCurrent/3, BGCOLOR);
      display->drawLine(eyeRx + eyeRwidthCurrent/2, eyeRy + eyeRheightCurrent/2, eyeRx + eyeRwidthCurrent/2 + eyeRwidthCurrent/4, eyeRy + eyeRheightCurrent/2 - eyeRwidthCurrent/8, BGCOLOR);
    }
  } else {
  ...
  if (happyHeart){/* No changes needed here, handled in drawing section */} else {
      // If not happyHeart, reset eye size to default
      eyeLwidthNext = eyeLwidthDefault;
      eyeLheightNext = eyeLheightDefault;
      eyeRwidthNext = eyeRwidthDefault;
      eyeRheightNext = eyeRheightDefault;
